...
from tkinter import messagebox, ttk  # Importer ttk for progresjonsindikatorer
import random
import os
import sys
import json
...
class SignGame:
  def __init__(self, root, base_folder):
      ...
      # **Ny funksjonalitet 3: Variabel for vanskelighetsgrad**
      self.current_difficulty = "easy"  # Legger til en variabel for å spore nåværende vanskelighetsgrad
  ...
  def load_categories(self):
      ...
      self.assign_category_colors()  # **Ny funksjonalitet 1: Fargekodet kategorisystem**
  
  def assign_category_colors(self):
      """Assign a unique color to each category."""
      colors = ["#FFB3BA", "#BAFFC9", "#BAE1FF", "#FFFFBA", "#FFDFBA", "#E0BBE4"]  # Liste med farger
      self.category_colors = {}
      for i, category in enumerate(self.categories):
          self.category_colors[category] = colors[i % len(colors)]  # Tildel farge basert på kategoriindeks
  ...
  def create_category_button(self, category):
      """Create a button for each category with percentage display and progress bar."""
      frame = tk.Frame(self.root, bg="#b0bec5")
      frame.pack(pady=10)

      # **Ny funksjonalitet 1: Kategorieknapp med spesifikk farge**
      button = tk.Button(frame, text=category, command=lambda c=category: self.use_category(c), 
                         ...
                         bg=self.category_colors[category])  # Bruk tildelt farge
      ...
      # **Ny funksjonalitet 2: Visuell fremgangsindikator med Progressbar**
  ...
  def use_all_categories(self):
      ...
      self.answered_questions = 0  # Reset answered questions for new quiz
      self.start_quiz()
  ...
  def check_answer(self):
      ...
      if user_input == self.correct_answer.lower():
          ...
          self.adjust_difficulty()  # **Ny funksjonalitet 3: Juster vanskelighetsgrad**
          self.load_new_image()
      else:
  ...
  def give_hint(self):
      """Give the player a hint by showing part of the correct answer based on difficulty."""
      if self.current_difficulty == "easy":
          hint_text = self.correct_answer[:3] + "..."  # Show first three letters
      elif self.current_difficulty == "medium":
          hint_text = self.correct_answer[:2] + "..."  # Show first two letters
      else:
          hint_text = self.correct_answer[0] + "..."  # Show only the first letter
  ...
  def update_labels(self):
      ...
      # **Ny funksjonalitet 4: Visuell belønning for streaks**
      if self.streak >= 5:
          streak_emoji = "🔥" * (self.streak // 5)  # Fire emoji for every 5 in the streak
